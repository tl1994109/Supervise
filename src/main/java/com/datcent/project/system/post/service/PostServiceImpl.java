package com.datcent.project.system.post.service;
...
public class PostServiceImpl implements IPostService
{
     ...
     */
    @Override
    public List<Post> selectPostsByPersonId(Long personId)
    {
        List<Post> userPosts = postMapper.selectPostsByPersonId(personId);
        List<Post> posts = postMapper.selectPostAll();
        if(userPosts!=null && userPosts.size()>0){
            for (Post post : userPosts)
            {
                for (Post userRole : posts)
                {
                    if (post.getPostId().longValue() == userRole.getPostId().longValue())
                    {
